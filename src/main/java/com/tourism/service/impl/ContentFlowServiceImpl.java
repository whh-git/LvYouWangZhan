...
public class ContentFlowServiceImpl implements ContentFlowService {
    ...
    //模糊查询景点
    public void updateCF(String title) {
        title="%"+title+"%";
        //查找所有于查询关键字相关的推荐景点
       List<ContentFlow> contentFlowList = contentFlowDao.findByContent(title);
       //遍历增加推荐值
        for (ContentFlow a:contentFlowList) {
            ContentFlow contentFlow=this.findById(a.getId());
           Integer o_p= contentFlow.getPupNo();
           Integer o_t_p= contentFlow.getTodayPupNo();
           //如果推荐值为空，给初始值为0
           if(o_t_p==null){
               o_t_p=0;
           }
           ...
           //如果今日搜索超过三次后，每搜索一次增加推荐值为10
            if(o_t_p >2){
                o_p=o_p+9;
            }
            contentFlow.setPupNo(o_p+1);
            contentFlow.setTodayPupNo(o_t_p+1);
            contentFlowDao.save(contentFlow);
        }

    }


    public List<ContentFlow> findByTitle(Object title) {

        String   sql="";
        String queryString = "from ContentFlow s where s.title like'%"+title+"%'";
        Query query = null;
        return query.list();
    }
